...
while start_time_count > end_time_count:
    ...
    url_temp = url = 'https://min-api.cryptocompare.com/data/histohour?fsym=BTC&tsym=USD&limit=24&toTs={}'.format(start_time_count)
    ...
    if round_count%500 is True:
        ...
        time.sleep(5)
        continue
    # always be careful about rate limits. The limit is closer to 3 seconds, but doesn't hurt to be safe
    # when you can afford to be

# writing the mostly un-touched data to csv for later ETL

master_df.to_csv('btc_hourly_data.csv', mode = 'w+')
